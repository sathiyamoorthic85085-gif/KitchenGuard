...
app$5.post("/reading", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const { sensor_type, value, unit } = await c.req.json();
  if (!sensor_type || value === void 0 || !unit) {
  ...
  await db.prepare(
    `
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `
  ).bind(user.id, sensor_type, value, unit, now, now).run();
...
app$4.get("/", async (c) => {
  ...
  const db = c.env.DB;
  const settings = await db.prepare("SELECT * FROM user_settings WHERE user_id = ? LIMIT 1").bind(user.id).first();
  if (!settings) {
    return c.json({
      settings: {
        lpg_threshold: 50,
...
app$4.put("/", async (c) => {
  ...
  const db = c.env.DB;
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const existing = await db.prepare("SELECT id FROM user_settings WHERE user_id = ? LIMIT 1").bind(user.id).first();
  if (existing) {
    await db.prepare(
      `
...
app$3.get("/rules", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const db = c.env.DB;
  const rules = await db.prepare("SELECT * FROM automation_rules WHERE user_id = ? ORDER BY id").bind(user.id).all();
  return c.json({ rules: rules.results });
});
app$3.post("/analyze", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  ...
  const rules = await db.prepare("SELECT * FROM automation_rules WHERE user_id = ? AND is_enabled = 1").bind(user.id).all();
  ...
  for (const reading of readings.results) {
    ...
    const sensorRules = rules.results.filter(
      ...
      if (shouldTrigger) {
        ...
        await db.prepare(
          `
            ...
            VALUES (?, ?, ?, 0, ?, ?, ?)
          `
        ).bind(user.id, rule.action_device, newState, now, now, now).run();
...
app$3.post("/process", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const { sensor_type, value } = await c.req.json();
  if (!sensor_type || value === void 0) {
    return c.json({ error: "Missing required fields" }, 400);
  }
  const db = c.env.DB;
...
app$2.get("/states", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const db = c.env.DB;
  const states = await db.prepare("SELECT * FROM device_states WHERE user_id = ? ORDER BY device_type").bind(user.id).all();
  return c.json({ states: states.results });
});
app$2.post("/toggle", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const { device_type, is_on, is_manual_override } = await c.req.json();
  if (!device_type || is_on === void 0) {
    return c.json({ error: "Missing required fields" }, 400);
  }
  const db = c.env.DB;
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const existing = await db.prepare("SELECT id FROM device_states WHERE user_id = ? AND device_type = ?").bind(user.id, device_type).first();
  if (existing) {
    await db.prepare(
      `
      ...
      `
    ).bind(is_on ? 1 : 0, is_manual_override ? 1 : 0, now, now, user.id, device_type).run();
  } else {
    await db.prepare(
      `
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `
    ).bind(user.id, device_type, is_on ? 1 : 0, is_manual_override ? 1 : 0, now, now, now).run();
  }
  return c.json({ success: true });
});
const app$1 = new Hono2();
app$1.get("/members", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const db = c.env.DB;
  const members = await db.prepare("SELECT * FROM family_members WHERE user_id = ? ORDER BY created_at DESC").bind(user.id).all();
  return c.json({ members: members.results });
});
app$1.post("/members", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
...
app$1.put("/members/:id", async (c) => {
  ...
  await db.prepare(
    ...
    `
  ).bind(name, relationship, email || "", phone || "", has_app_access ? 1 : 0, now, id, user.id).run();
  return c.json({ success: true });
});
app$1.delete("/members/:id", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const id = c.req.param("id");
  const db = c.env.DB;
  await db.prepare("DELETE FROM family_members WHERE id = ? AND user_id = ?").bind(id, user.id).run();
  return c.json({ success: true });
});
app$1.get("/devices", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const db = c.env.DB;
  const devices = await db.prepare("SELECT * FROM paired_devices WHERE user_id = ? ORDER BY created_at DESC").bind(user.id).all();
  return c.json({ devices: devices.results });
});
app$1.post("/devices/pair/start", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const pairingCode = Math.floor(1e5 + Math.random() * 9e5).toString();
  return c.json({ pairingCode, expiresIn: 300 });
});
app$1.post("/devices/pair/complete", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  ...
  const db = c.env.DB;
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const existing = await db.prepare("SELECT id FROM paired_devices WHERE device_id = ?").bind(device_id).first();
  if (existing) {
...
app$1.put("/devices/:id", async (c) => {
  ...
  await db.prepare(
    `
      ...
      WHERE id = ? AND user_id = ?
    `
  ).bind(device_name, is_online ? 1 : 0, now, now, id, user.id).run();
  return c.json({ success: true });
});
app$1.delete("/devices/:id", async (c) => {
  const user = c.get("user");
  if (!user?.id) {
    return c.json({ error: "Unauthorized" }, 401);
  }
  const id = c.req.param("id");
  const db = c.env.DB;
  await db.prepare("DELETE FROM paired_devices WHERE id = ? AND user_id = ?").bind(id, user.id).run();
  return c.json({ success: true });
});
const app = new Hono2();
...
app.use("/api/sensors/*", authMiddleware);
app.route("/api/sensors", app$5);
app.use("/api/settings/*", authMiddleware);
app.route("/api/settings", app$4);
app.use("/api/automation/*", authMiddleware);
app.route("/api/automation", app$3);
app.use("/api/devices/*", authMiddleware);
app.route("/api/devices", app$2);
app.use("/api/family/*", authMiddleware);
app.route("/api/family", app$1);
